<template>
  <div>
    <div class="wrapper-map">
      <div class="map" ref="map" @click="clickCoord"></div>
    </div>
    <div ref="infoWindow" class="info-window">
      <template>
        <slot name="infowindow" class="boxInfo" v-for="item in markers" :item="item"></slot>
      </template>
    </div>
  </div>
</template>
...
import { Component, Prop, Watch, Emit, Provide, Inject, Model } from 'vue-property-decorator';
...
export default class Maps extends Vue {
  ...
  // @Prop({ type: Object, default: null }) markerItem: IMarkerData
  ...
  innerMarkerItem: IMarkerData = null

  observer: MutationObserver = null;
  zoomChange: number;

  arrInfoWindows: { id: number; opened: boolean; info: google.maps.InfoWindow }[] = [];
  ...
  OnMarkerChanged() {
    ...
    const markersToDelete = this.innerMarkers.filter((x) => !this.markers.some((nm) => nm.id == x.id));
    const isFit = markersToDelete.length === this.innerMarkers.length;
    const markersToCreate = this.markers.filter((x) => !this.innerMarkers.some((om) => om.id == x.id));
    const markersToEdit = this.innerMarkers.filter((x) => this.markers.some((nm) => nm.id == x.id && this.isMarkerChanged(x, nm)));
    ...
    markersToEdit.forEach((x) => this.changeMarker(x, this.markers.find((em) => em.id == x.id)));
  ...
  changeMarker(innerMarker: IMarkerData, marker: MarkerTemplate): void {
    ...
    if (!innerMarker || !marker) {
      console.error('Нельзя изменить маркер. Исходный или данный маркер не определен');
    }
    innerMarker.marker.setDraggable(marker.draggable);
    innerMarker.marker.setPosition({ lat: marker.positionPoint.lat, lng: marker.positionPoint.lng });
  ...
  changeMarkerItem(innerMarkerItem: IMarkerData, marker: MarkerTemplate) {
    if (!innerMarkerItem || !marker) {
      console.error('Нельзя изменить маркер. Исходный или данный маркер не определен');
    }
    innerMarkerItem.marker.setDraggable(marker.draggable);
    innerMarkerItem.marker.setPosition({ lat: marker.positionPoint.lat, lng: marker.positionPoint.lng })
  }
  ...
  deleteMarkers(markersToDelete: IMarkerData[]) {
    ...
    this.innerMarkers = this.innerMarkers.filter((x) => !markersToDelete.some((dm) => dm.id == x.id));
    this.arrInfoWindows = this.arrInfoWindows.filter((aiw) => !markersToDelete.some((dm) => dm.id == aiw.id));
  }
  ...
  openInfoWindow(currentMarker: { id: number; marker: google.maps.Marker }) {
    ...
    exist = exist || {
      ...
      info: new google.maps.InfoWindow({
        content: this.contentTemplate[idx] == null ? '' : this.contentTemplate[idx].outerHTML,
        maxWidth: 400,
      }),
    };
  ...
  fitByMarkers() {
    ...
    for (let index = 0; index < this.innerMarkers.length; index++) {
      const loc = new google.maps.LatLng(this.innerMarkers[index].marker.position.lat(), this.innerMarkers[index].marker.position.lng());
      bounds.extend(loc);
    }
    // debugger
  ...
  paintRoute() {
    ...
    //   const loc = new google.maps.LatLng(this.route[0].points[index].lat, this.route[0].points[index].lng);
    //   bounds.extend(loc);
    // }
  ...
  calculateAndDisplayRoute(waypoints: any[]) {
    ...
    //     var end = new google.maps.LatLng(row[row.length - 1].lat, row[row.length - 1].lng);
  ...
  initMap() {
    ...
    google.maps.event.addListener(map, 'click', function(event) {
      ...
      // populate yor box/field with lat, lng
      self.$emit('getNewCoord', { lat: lat, lng: lng });
  ...
  mounted() {
    // let map = document.querySelector('#wrapper-map')
    // let mapPoint.innerHTML = `<div class="pointer" <img src="assets/img/map-marker.svg" > style="left: ${this.left}; top: ${this.top}"></div>`
    var htmlTmp = (this.$refs.infoWindow as HTMLElement).children;
    this.contentTemplate = htmlTmp;
    
    // console.log('mountedL!!@#WE');
    const self = this;
    this.observer = new MutationObserver((mutations: MutationRecord[], observer: MutationObserver): void => {
      var htmlTmp = (this.$refs.infoWindow as HTMLElement).children;
